...
func (it *stringIterator) Next() string { if !it.HasNext() { return "" }; v := it.items[it.index]; it.index++; return v }
...
func (it *networkIterator) Next() *libbox.NetworkInterface { if !it.HasNext() { return nil }; v := it.items[it.index]; it.index++; return v }
...
func (p *harmonyPlatform) StartDefaultInterfaceMonitor(listener libbox.InterfaceUpdateListener) error { return nil }
func (p *harmonyPlatform) CloseDefaultInterfaceMonitor(listener libbox.InterfaceUpdateListener) error { return nil }
func (p *harmonyPlatform) GetInterfaces() (libbox.NetworkInterfaceIterator, error) {
	...
	for _, iface := range interfaces {
		if iface.Name == "lo" || strings.Contains(strings.ToLower(iface.Name), "vpn") { continue }
		addresses := make([]string, 0)
		for _, address := range iface.Addrs() {
			prefix, err := netip.ParsePrefix(address.String()); if err != nil { continue }
			if prefix.Addr().IsLinkLocalUnicast() { continue }
...
func vpndirect_harmony_start(fd C.int, profile *C.char) C.int {
	...
	if service != nil { return -114 }
	if fd < 0 || profile == nil { return -22 }
	config := C.GoString(profile)
	if !strings.Contains(config, "\"inbounds\"") { return -22 }
	tunFD = int(fd)
	instance, err := libbox.NewService(config, &harmonyPlatform{})
	if err != nil { tunFD = -1; return -22 }
	if err = instance.Start(); err != nil { _ = instance.Close(); tunFD = -1; return -5 }
	service = instance
	return 0
}
...
func vpndirect_harmony_stop() C.int {
	mu.Lock(); defer mu.Unlock()
	if service == nil { tunFD = -1; return 0 }
	err := service.Close(); service = nil; tunFD = -1
	if err != nil { return -5 }
	return 0
}
