...
public class ExtensionEnvironments: ObservableObject {
    ...
    public func reload() async {
        ...
        defer { extensionProfileLoading = false }

        if let newProfile = try? await ExtensionProfile.load() {
            // Always register — replacing extensionProfile drops the old observer.
            newProfile.register()
    ...
    public func ensureExtensionProfileReady() async throws {
        ...
        guard extensionProfile != nil else {
            throw NSError(
                domain: "ExtensionEnvironments",
                code: -1,
                userInfo: [NSLocalizedDescriptionKey: String(localized: "Не удалось подготовить VPN-профиль.")]
            )
        }
    }
