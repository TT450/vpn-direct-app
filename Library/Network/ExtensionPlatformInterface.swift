...
public class ExtensionPlatformInterface: NSObject, LibboxPlatformInterfaceProtocol, LibboxCommandServerHandlerProtocol {
    ...
    private func openTun0(_ options: LibboxTunOptionsProtocol?, _ ret0_: UnsafeMutablePointer<Int32>?) async throws {
        ...
        if options.getAutoRoute() {
            ...
            let dnsServers = try options.getDNSServerAddress()
            var servers: [String] = []
            while dnsServers.hasNext() {
                servers.append(dnsServers.next())
            }
            if servers.isEmpty {
                servers = ["1.1.1.1"]
            }
    ...
    // MARK: - Libbox 1.14+ platform stubs (unused on iOS NE path)

    public func checkPlatformShell() throws {
        throw NSError(domain: "ExtensionPlatformInterface", code: 0, userInfo: [NSLocalizedDescriptionKey: "Platform shell unsupported"])
    }
    ...
    public func createBridge(_: LibboxBridgeOptions?) throws -> any LibboxBridgeSessionProtocol {
        throw NSError(domain: "ExtensionPlatformInterface", code: 0, userInfo: [NSLocalizedDescriptionKey: "Bridge unsupported"])
    }

    public func startNeighborMonitor(_: (any LibboxNeighborUpdateListenerProtocol)?) throws {}

    public func closeNeighborMonitor(_: (any LibboxNeighborUpdateListenerProtocol)?) throws {}

    public func lookupSFTPServer(_ error: NSErrorPointer) -> String {
        error?.pointee = NSError(domain: "ExtensionPlatformInterface", code: 0, userInfo: [NSLocalizedDescriptionKey: "SFTP unsupported"])
        return ""
    }

    public func readSystemSSHHostKey(_ error: NSErrorPointer) -> String {
        error?.pointee = NSError(domain: "ExtensionPlatformInterface", code: 0, userInfo: [NSLocalizedDescriptionKey: "SSH host key unsupported"])
        return ""
    }

    public func lookupUser(_: String?) throws -> LibboxPlatformUser {
        throw NSError(domain: "ExtensionPlatformInterface", code: 0, userInfo: [NSLocalizedDescriptionKey: "lookupUser unsupported"])
    }

    public func openShellSession(
        _: LibboxPlatformUser?,
        command _: String?,
        environ _: (any LibboxStringIteratorProtocol)?,
        term _: String?,
        rows _: Int32,
        cols _: Int32
    ) throws -> any LibboxShellSessionProtocol {
        throw NSError(domain: "ExtensionPlatformInterface", code: 0, userInfo: [NSLocalizedDescriptionKey: "Shell session unsupported"])
    }
    ...
    public func connectSSHAgent(_: UnsafeMutablePointer<Int32>?) throws {
        throw NSError(domain: "ExtensionPlatformInterface", code: 0, userInfo: [NSLocalizedDescriptionKey: "SSH agent unsupported"])
    }

    public func triggerNativeCrash() throws {
        throw NSError(domain: "ExtensionPlatformInterface", code: 0, userInfo: [NSLocalizedDescriptionKey: "triggerNativeCrash not enabled"])
    }
}
