...
///
/// Identity policy (TheTochka/Happ + ChatGPT honesty):
/// - Every endpoint instance is built (never collapse by display name).
/// - Tags uniquify from display names (`Germany`, `Germany-2`) for UI/flags.
/// - Topology refs resolve by xrayTag / loc-index / unique name; ambiguous name refs fail closed.
public enum SingBoxGraphBuilder {
    ...
    public struct GraphResult {
        ...
        public let rejected: [RejectedLeaf]
    ...
    public static func build(from subscription: NormalizedSubscription) throws -> GraphResult {
        ...
        for (locationIndex, location) in subscription.locations.enumerated() {
            ...
            for (entryIndex, endpoint) in location.endpoints.enumerated() {
                ...
                // TheTochka/Happ UX: tag from display name (Germany → Germany-2). Engine identity is the
                // uniquified tag; display name is only a soft reference (ambiguous only if topology cites it).
                ...
                let built = BuiltLeaf(
                    ...
                    object: tagged,
                    tag: leafTag,
                    isEndpoint: isTunnelEndpoint(tagged)
                )
                builtLeaves.append(built)
                memberTags.append(leafTag)

                // Authoritative: xrayTag + scoped loc/index. Display name registered for detour compatibility.
                ...
                registerReference(leafTag, tag: leafTag)
            }
        ...
        for (location, memberTags) in locationBuilt {
            ...
            case .random:
                // No faithful Core outbound for Clash load-balance/random (TheTochka never imported these).
            ...
            case .fallback:
                // Libertea / Clash fallback: Core has no native failover outbound. Closest honest mapping is
                // urltest (latency probe) with an explicit warning — never silently pretend perfect parity.
                outbounds.append(urltestOutbound(
                    tag: locationTag,
                    members: memberTags,
                    url: location.healthCheckURL,
                ...
                warnings.append(
                    "location '\(location.name)' strategy=fallback → urltest (Core has no native fallback; Libertea-compat approximation)"
                )
            case .single:
        ...
        // TheTochka/Happ graph root: selector `proxy` + urltest `auto` over location tags.
        // REQ-P133: sing-box-lx Outbound(tag) falls back to endpoint.Get(tag), so WG endpoint
        // tags are valid selector/urltest members without a bridge outbound.
    ...
    public static func build(fromShareNodes nodes: [NormalizedNode]) throws -> GraphResult {
    ...
    private static func isTunnelEndpoint(_ leaf: [String: Any]) -> Bool {
        let type = (leaf["type"] as? String)?.lowercased() ?? ""
        switch type {
        case "wireguard", "amneziawg", "openvpn-client", "openconnect", "tailscale":
            return true
        default:
            return false
        }
