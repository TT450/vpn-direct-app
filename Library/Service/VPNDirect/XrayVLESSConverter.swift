...
enum XrayVLESSConverter {
    static func convert(_ xray: [String: Any], fallbackTag: String) -> [String: Any]? {
        ...
        if network != "tcp" && network != "raw" && !network.isEmpty {
            guard var transport = xrayTransport(network: network, stream: stream) else { return nil }
            // Reality + explicit stream-one → auto (TheTochka / lx framing quirk — keep until Core proves otherwise).
            if security == "reality",
