...
.PHONY: core-bootstrap libbox libbox-backup-stock libbox-restore-stock libbox-install check-fixtures check-abi check-capability-proofs

core-bootstrap:
	"$(ROOT)/scripts/bootstrap_core.sh"

libbox-backup-stock:
	@if [ -d "$(ROOT)/Libbox.xcframework" ] && [ ! -d "$(ROOT)/Libbox.xcframework.stock" ]; then \
		echo "Backing up stock Libbox → Libbox.xcframework.stock"; \
		ditto "$(ROOT)/Libbox.xcframework" "$(ROOT)/Libbox.xcframework.stock"; \
	elif [ -d "$(ROOT)/Libbox.xcframework.stock" ]; then \
		echo "Stock backup already present at Libbox.xcframework.stock"; \
	else \
		echo "No Libbox.xcframework to back up"; exit 1; \
	fi

libbox: libbox-backup-stock
	"$(ROOT)/scripts/build_libbox.sh"

libbox-install: libbox
	@echo "Libbox.xcframework installed at $(ROOT)/Libbox.xcframework"

libbox-restore-stock:
	@if [ ! -d "$(ROOT)/Libbox.xcframework.stock" ]; then \
		echo "Missing Libbox.xcframework.stock"; exit 1; \
	fi
	rm -rf "$(ROOT)/Libbox.xcframework"
...
check-capability-proofs:
	"$(ROOT)/scripts/check_capability_proofs.sh"
...
build_macos_pkg_apple: archive_macos_standalone_apple export_macos_standalone_apple
	...
	pkgbuild --root "build/pkgroot-arm64" \
		...
		--identifier com.vpndirect.vpndirectapp.standalone \
